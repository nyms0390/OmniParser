...
from omnitool.gradio.core.agents.image_utils import (
    _crop_b64,
    _resize_b64,
    _compare_screens,
)
...
class BaseAgent(ABC):
    ...
    def _capture_screen(self) -> Dict[str, Any]:
        ...
        resized_b64 = _resize_b64(screenshot_b64, self.screenshot_max_width)
    ...
    def _parse_response(
        ...
        try:
            json_str = _extract_data(response_text, "json")
    ...
    def _verify_step(
        ...
        return {
            ...
            "screen_unchanged": _compare_screens(
    ...
    def _detect_repeated_actions(self, threshold: int = 3) -> bool:
        ...
        infos = [_extract_primary_action(e.get("tool_calls", [])) for e in recent]
        ...
        *aggregate["store_as"]*. Appends a human-readable status line to *results*.
        ...
        for item in items:
            ...
            try:
                ...
                pass  # non-numeric field; expected
    ...
    def _format_recent_actions(self, n: int = 5) -> str:
        ...
        lines = []
        for entry in recent:
            info = _extract_primary_action(entry.get("tool_calls", []))
            coord_str = (
